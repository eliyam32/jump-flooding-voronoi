...
uniform sampler2DRect tex0,tex1; /* buffers */
...
void main()
{
	...
	if( iter == 0 )
		...
		fragData0 = texture2DRect( tex1, gl_FragCoord.st - vec2(0.5) );
	...
	for( i = 0; i < 8; ++i )
	{
		...
		if( iter == 0 )
			...
			neighbor0 = texture2DRect( tex1, nCoord[i] - vec2(0.5) );
